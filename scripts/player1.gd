...
func _ready():
	...
	if is_multiplayer_authority():
		var random_animation = str(randi_range(1, 4))
		sprite.play(random_animation)
		set_player_sprite.rpc(random_animation)

@rpc("any_peer", "call_local", "reliable")
func set_player_sprite(animation_name: String):
...
func move_to_cell(target_cell: Vector2i):
	...
	stamina -= move_cost
	var highlight_layer = tilemap.get_parent().get_node("HighlightLayer")
	highlight_layer.clear()
	highlight_layer.set_cell(target_cell, 0, Vector2i(20, 1))
